...
struct HomePageView: View {
    ...
    private func initializeFolderState() {
        ...
        if isMainFolderPresent {
            lockManager.isLocked = false // Set isLocked to false if "MainFolder" exists
            print("MainFolder Present")
        } else if isEncryptedFolderPresent {
            lockManager.isLocked = true // Set isLocked to true if "MainFolder.encrypted" exists
            print("MainFolder.encrypted Present")
        } else {
            FolderManager.createFolderStructure()
            lockManager.isLocked = false // Set isLocked to false after creating the folder structure
        }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            inactivityTimerManager.startTimer(inactivityThreshold: 300) { // After 5 minutes will lock
                Task {
                    do {
                        print("Timer ran out")
                        ...
                        print("Error during encryption or sign out: \(error)")
                    }
                }
            }
        }
        ...
        TabView {
            NavigationView {
                VStack {
                    ...
                    Button(action: {
                        lockManager.isLocked.toggle() // Unlock the folder
                        if !lockManager.isLocked {
                            do {
    ...
    var lockButton: some View {
        Button(action: {
            lockManager.isLocked.toggle() // Lock or unlock the folder
            Task {
                do {
                    if lockManager.isLocked {
                        let publicKey = try await getPublicKey()
        ...
        .padding(.trailing)
        .disabled(lockManager.isLocked) // Disable the lock button when the folder is locked
        .alert(isPresented: $showAlert) {
            Alert(title: Text(alertMessage), dismissButton: .default(Text("OK")))
        }
    }
